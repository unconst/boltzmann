...
class ModelArgs:
    ...
    attn_fn: Literal["sdpa", "flash"] = "flash"
...
class Attention(nn.Module):
    ...
    def _fa_attention_with_seqlens(self, xq, xk, xv, seqlens) -> torch.Tensor:
        b = xq.shape[0]
        cu_seqlens = (
            torch.concat([torch.tensor([0]).to(xq.device), seqlens.cumsum(0)], dim=0)
            .to(torch.int32)
            .to(xq.device)
        )
    ...
    def self_attention(
        ...
        if self.attn_fn == "sdpa":
            ...
            if not flash_attn_available:
                raise RuntimeError(
                    "Flash attention is not available. Please install flash_attn."
                )
            if seqlens is not None:
